...
export const copySample = async (title, category) => {
...
export const parseFields = (category, fields) => {
  ...
  const titlesArray = [];
  const optionsArray = [];
  const fieldLinksArray = [];
  fields.forEach((el) => {
    if (el.fullName === category) {
      el.tags.forEach((tg) => {
        tagsArray.push(tg.tag);
        titlesArray.push(tg.title);
        optionsArray.push(tg.options);
        fieldLinksArray.push(tg.fieldLink);
      });
    }
  });
  return { tagsAndTitlesArray: [tagsArray, titlesArray], optionsArray, fieldLinksArray };
};

export const pasteTags = async (spreadsheetId, sheetId, values, sheetTitle, fieldLinksArray) => {
        ...
        requests: [
          ...
          },
          {
            updateCells: {
              rows: [
                {
                  values: fieldLinksArray.map((fieldLink) => ({
                    userEnteredFormat: {
                      textFormat: {
                        bold: true,
                        link: {
...
export const setEnumValues = async (spreadsheetId, sheetId, optionsArray, category) => {
  ...
  optionsArray.forEach((options) => {
    let type = 'ONE_OF_RANGE';
    if (options.length === 0) type = 'TEXT_CONTAINS';
    requests.push({
      setDataValidation: {
        range: {
          sheetId,
 ...
 * Writes the sheet id into a hidden area (column)
 * @param sheetId
 * @param spreadsheetId
 */
...
const createSpreadSheet = async (category, title, fullName) => {
  ...
  const fields = await api('mongo').get(`/fields/full/${preparedFullname}`)
    ...
    .catch((error) => {
      console.error(error);
      throw new Error('Error in createSpreadSheet func');
    });
  const newSpreadSheet = await copySample(title, category);
