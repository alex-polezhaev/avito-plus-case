...
export const getImageLinks = async (acc, folder, timeToSleep) => {
  ...
  // Collect the image paths from the folder
  ...
  if (error === 403) { return '❗️Increase your Disk storage'; }
  ...
  // Publish the photos using their paths
  await Promise
    .all(pathsArray?.map((path) => yandexAPI
      .put(`/publish?path=${encodeURIComponent(path)}`)))
    .catch((err) => { error = err; });

  if (error) { return '❗️A problem occurred while uploading the photos - contact support'; }
